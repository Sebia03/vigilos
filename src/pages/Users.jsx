...
export default function Users({ currentUser }) {
  ...
  const [loading, setLoading]   = useState(false);
  const [modalUser, setModalUser] = useState(null); // null = fermé, {} = nouveau, {id,...} = édition
  ...
  const getRoleBadge = (role) => {
    if (role === "superadmin") return (
      <span className="rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-0.5 text-[11px] font-medium text-cyan-400">
        Super Admin
      </span>
    );
    return (
    ...
    <div className="animate-fadeIn p-6">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-gray-800 bg-gray-950 p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-gray-400">
              Supprimer <span className="text-white font-medium">{deleteConfirm.email}</span> ?
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">{users.length} utilisateur{users.length > 1 ? "s" : ""} enregistré{users.length > 1 ? "s" : ""}</p>
        </div>
        <button
          onClick={() => setModalUser({})}
          className="flex items-center gap-2 rounded-xl bg-cyan-500/20 border border-cyan-500/30 px-4 py-2.5 text-sm font-medium text-cyan-400 transition hover:bg-cyan-500/30"
        >
